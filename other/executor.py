...
class CollectorThread(QtCore.QThread):
    ...
    def run(self):
        if not self.dbf_fls_path or not self.mdb_out_file:
            ...
            return
        try:
            run_collection(self.dbf_fls_path, self.mdb_out_file)
        except Exception as err:
...
class MainWindow(QtGui.QMainWindow):
    ...
    def get_dbf_dir(self):
        dir_path = unicode(QtGui.QFileDialog(self).getExistingDirectory(self, widget_titles.choose_dbf_dir, project_dir, options=QtGui.QFileDialog.DontUseNativeDialog))
        if os.path.isdir(dir_path):
            self.dbf_files_dir = dir_path.replace('/', '\\')
            self.src_dbf_widget.set_lbl_text(self.dbf_files_dir)

    def get_src_mdb_file(self):
        file_path = unicode(QtGui.QFileDialog(self).getOpenFileName(self, widget_titles.choose_src_mdb, project_dir, u'Valid files (*.mdb);; All files (*)', options=QtGui.QFileDialog.DontUseNativeDialog))
