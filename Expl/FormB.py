...
temp_dbf = u"%s\\tempDbase.mdb" % workDir

class ExpFormaB(object):
    def __init__(self, expdb, crtabdb = temp_dbf):
        self.__codsdict = {}
    ...
    def makeRowCodes(self):
        ...
        """
        lcrows = self.__selectSprav(u'select LandCode, NumberGraf from LandCodes')
        rcrows = self.__selectSprav(u'select Code, RowNames from RowCodes')
        for row in lcrows:
    ...
    def createExpDict(self):
        self.formb_rows = self.__selectSprav(u'Select Forma22, ValueForm22, SignData from S_F22All')
        ...
        expDict[u'04'] = self.sumDictValues(*map(lambda x: expDict[x], [u'05',u'06',u'07',u'08',u'09', u'10', u'11']))
        expDict[u'01'] = self.sumDictValues(expDict[u'01'],expDict[u'02'])
        expDict[u'15'] = self.sumDictValues(expDict[u'15'],expDict[u'16'])
        expDict[u'40'] = expDict[u'18'].copy()
        expDict[u'18'] = self.sumDictValues(expDict[u'18'],expDict[u'19'])
        expDict[u'43'] = expDict[u'22'].copy()
        expDict[u'22'] = self.sumDictValues(expDict[u'22'],expDict[u'23'],expDict[u'24'])
        expDict[u'25'] = self.sumDictValues(*map(lambda x: expDict[x], [u'01',u'03',u'04',u'12',u'13',u'14',u'15',u'17',u'18',u'20',u'21',u'22']))
        expDict[u'28'] = self.sumDictValues(expDict[u'25'],expDict[u'27'])
        expDict[u'28'] = self.diffDictValues(expDict[u'28'],expDict[u'26'])
        expDict[u'32'] = self.sumDictValues(expDict[u'33'],expDict[u'34'],expDict[u'35'],expDict[u'36'])
        expDict[u'37'] = self.sumDictValues(expDict[u'01'],expDict[u'03'])
        expDict[u'39'] = self.sumDictValues(*map(lambda x: expDict[x], [u'12',u'13',u'14',u'15',u'17']))
        ...
        expDict[u'38'] = self.sumDictValues(expDict[u'04'],expDict[u'23'])
        expDict[u'41'] = expDict[u'20'].copy()
        return expDict

    def makeInsParameters(self, sortby, tempBy = u'F22'):
        """
        Makes parameters for insert into FormaB
        :param sortby: Name of Forma22 in unicode format
        ...
        tempCTdata =[]
        if tempBy == u'F22':
            tempCTdata = self.selectF22Cash(sortby)
        else:
            tempCTdata = self.selectoCtrCash(sortby)
        if tempCTdata == []:
            return params
        else:
            params[u'f_1'] = sum([row[0] for row in tempCTdata])
            for i in dependOfCodes:
                if i[1] == 2:
                    sortedtemp = [row for row in tempCTdata if row[2] in self.__codes[i[0]]]
                ...
                if i[1] == 1:
                    params[i[0]] = sum([row[0] for row in tempCTdata if row[1] in self.__codes[i[0]]])
                if i[1] == 4:
                    params[i[0]] = sum([row[0] for row in tempCTdata if row[4] in self.__codes[i[0]]])
                if i[1] == 3:
                    params[i[0]] = sum([row[0] for row in tempCTdata if row[3] in self.__codes[i[0]]])
    ...
    def runExpB(self):
        self.expdict = self.createExpDict()
    ...
    @property
    def connectCrtab(self):
        try:
            self.__ctconn = pyodbc.connect(self.__crtabdb, autocommit = True, unicode_results = True)
