...
default_sheet_name = 'Expl_export'
...
def exp_matrix(matrix, start_f, start_r, templ_path, save_as, is_xls_start, sh_name = None):
    ...
    try:
        w_book.save(save_as)
        if is_xls_start:
            os.system('start excel.exe %s' % save_as)
    except IOError:
...
def exp_single_fa(fa_data, f22_ind, obj_name, expl_file, a_l, a_n, a_obj_l, a_obj_n, a_path, a_sh_name, is_xls_start, **kwargs):
    excel_path = os.path.dirname(expl_file)+ '\\%s_xlsx_files' % os.path.basename(expl_file)[4:-4]
    if not os.path.exists(excel_path):
        os.makedirs(excel_path)
    dest_filename = '%s\\%s.xlsx'%(excel_path, f22_ind)
    ...
    if not os.path.isfile(dest_filename):
        ...
        w_book.save(filename=dest_filename)
        if is_xls_start:
            os.system('start excel.exe %s' % str(dest_filename))
        # raise XlsIOError(3,dest_filename)


if __name__ == '__main__':
    print get_xl_letter(234, 'M')
    print get_xl_letter(24, 'A')